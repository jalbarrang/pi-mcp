...
const store = {
  ...
  save: async (name: string, value: Record<string, unknown>) => { values.set(name, value); },
  clear: async () => {},
};
test("persists OAuth material and opens the authorization URL", async () => {
  const opened: URL[] = [];
  const provider = new OAuthProvider("server", "http://127.0.0.1:1/callback", store, { open: async (url) => { opened.push(url); } }, () => {});
  ...
  expect(provider.clientMetadata).toMatchObject({ client_name: "pi-mcp", token_endpoint_auth_method: "none" });
...
test("surfaces the URL when opening a browser fails", async () => {
  let surfaced = "";
  const provider = new OAuthProvider("fallback", "http://127.0.0.1:1/callback", store, { open: async () => { throw new Error("no browser"); } }, (url) => { surfaced = url.href; });
  await provider.redirectToAuthorization(new URL("https://auth.example/authorize"));
