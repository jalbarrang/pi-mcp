...
class Fake implements McpConnectionPort {
  ...
  async callTool() {
    return { content: [{ type: "text" as const, text: "bad" }], isError: true };
  }
  async close() {
    this.closed++;
  }
}
test("maps MCP errors to throws", () =>
  expect(callTool(new Fake(), "x", {})).rejects.toThrow("bad"));
test("stops manager idempotently", async () => {
  const fake = new Fake();
  const manager = new ConnectionManager(() => fake);
  manager.connections.set("x", fake);
