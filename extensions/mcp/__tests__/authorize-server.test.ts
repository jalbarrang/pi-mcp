...
class FakeConnection implements McpConnectionPort {
  ...
  async retryConnection() {
    if (++this.retries < 3) throw unauthorized();
    this.state = "ready" as never;
  }
  async beginAuthorization() {
    return { waitForCode: () => this.wait(), close: async () => {} };
  }
...
test("authorizes, exchanges the callback code, and reconnects", async () => {
  ...
  expect([connection.finished, connection.retries, connection.state] as (
    | number
    | string
  )[]).toEqual([1, 3, "ready"]);
});
...
test("masks callback timeout details", async () => {
  const connection = new FakeConnection();
  connection.wait = async () => {
    throw new Error("Authorization timed out");
  };
  await expect(authorizeServer(connection, new AbortController().signal)).rejects.toThrow(
