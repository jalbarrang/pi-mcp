...
const spec: ServerSpec = { name: "remote", kind: "http", url: "https://x", headers: {}, enabled: true, lazy: true };
class AuthFake implements McpConnectionPort {
  state = "idle" as "idle" | "needs-auth" | "ready";
  authorizations = 0;
  syncs = 0;
  retries = 0;
  async connect() { this.state = "needs-auth"; }
  async retryConnection() { if (++this.retries < 3) throw new Error("auth"); this.state = "ready"; }
  async beginAuthorization() { this.authorizations++; return { waitForCode: async () => "code", close: async () => {} }; }
  async finishAuth() {}
  isAuthorizationError() { return true; }
...
test("connect command authorizes a needs-auth server before syncing", async () => {
  ...
  const runtime = { catalog: () => new Map([[spec.name, spec]]), manager, tools: new ToolCatalog(), connect: async () => { fake.syncs++; }, reload: async () => "" };
...
test("reload diff reports add, changed, removed, and unchanged entries", () => {
  ...
  const result = diffCatalogs(new Map([["same", unchanged], ["changed", changed], ["removed", spec]]), new Map([["same", unchanged], ["changed", { ...changed, lazy: false }], ["added", spec]]));
  expect(result).toEqual({ added: ["added"], changed: ["changed"], removed: ["removed"], unchanged: ["same"] });
});
