...
const unsupported = new Set(["patternProperties", "allOf", "if", "then", "else", "definitions", "$defs"]);
...
export function sanitizeInputSchema(input: Record<string, unknown>): Sanitized {
  ...
  for (const [key, value] of Object.entries(input)) if (key !== "$schema" && !unsupported.has(key)) schema[key] = value;
