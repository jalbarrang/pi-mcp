...
export class BrowserOpen implements BrowserPort {
  constructor(private readonly exec: Exec, private readonly platform = process.platform) {}
  async open(url: URL) {
    const [command, args] = this.platform === "darwin" ? ["open", [url.href]] :
      this.platform === "win32" ? ["start", [url.href]] : ["xdg-open", [url.href]];
